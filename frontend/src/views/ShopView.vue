<template>
    <div class="container">
        <nav class="categories">
            ...
            <button class="category" v-for="category in categories" :key="category.id" @click="selectCategory(category.categoryId)">
                {{ category.name }}
            </button>
        </nav>
        <div class="subNav">
            ...
            <button id="searchButton" @click="selectSearch(searchQuery)">Search</button>
            <button id="cartModal" @click="openCartModal" type="button" class="btn btn-default btn-sm">
        ...
        <table class="table">
            ...
            <tbody>
                <tr v-for="(product, index) in filterProducts()" :key="index">
                    ...
                    <button id="addToCartButton" @click="addToCart(product, product.quantity)">Add to cart</button>
    ...
    methods: {
        ...
        filterProducts() {
            ...
            if (this.selectedCategory !== '' && filteredProducts.length !== 0) {
                filteredProducts = filteredProducts.filter(product => {
                    return product.category.categoryId === this.selectedCategory;
                });
                if (filteredProducts.length !== 0) {
                    Notiflix.Notify.success(`Found ${filteredProducts.length} products`);
                }
            ...
            if ((this.selectedCategory !== '' || this.search !== '') && filteredProducts.length === 0) {
...
#addToCartButton:hover, #searchButton:hover, #cartModal:hover, #searchButton:hover {
